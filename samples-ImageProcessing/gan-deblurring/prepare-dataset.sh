...
while IFS= read -d '' img; do
  ...
  echo "Creating $DATASET_DIR/big-$oname"
  convert "$img" -resize $RES $OPS -extent $RES "$DATASET_DIR/good-$oname"

  echo "Creating $DATASET_DIR/bad-$oname"
  # scaling down good image to 25%
  convert "$DATASET_DIR/good-$oname" -resize 25% $OPS "$DATASET_DIR/bad-$oname"
  # scale up bad image to 400% - as a blurred example
  convert "$DATASET_DIR/bad-$oname" -resize 400% $OPS "$DATASET_DIR/bad-$oname"
